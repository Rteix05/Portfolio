    ...
    <main className="bg-[#0c0c0c] text-white min-h-screen">
      ...
      <section ref={heroRef} className="min-h-screen pt-32 pb-20 px-6 md:px-16 flex flex-col justify-end">
        <div className="max-w-7xl mx-auto w-full">
          <div className="flex flex-col md:flex-row gap-8 mb-16 items-end">
            <div
              className="w-48 h-60 md:w-64 md:h-80 rounded-2xl overflow-hidden shrink-0 border border-white/10"
              data-hero
            >
            ...
            <div className="flex-1 flex flex-col gap-4" data-hero>
              <div className="flex flex-wrap gap-2">
                <span className="text-xs px-3 py-1 rounded-full border border-white/20 text-white/50 uppercase tracking-widest">
                  Troyes / Paris
                </span>
                <span className="text-xs px-3 py-1 rounded-full border border-white/20 text-white/50 uppercase tracking-widest">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-auto pt-8 border-t border-white/10">
                {[
                  ...
                  <div key={k}>
                    <p className="text-xs text-white/30 uppercase tracking-widest mb-1">{k}</p>
          ...
          <div data-hero className="border-t border-white/10 pt-8 flex items-end justify-between">
            <div>
              <p className="text-white/60 font-serif text-2xl md:text-3xl italic">Rafael Teixeira</p>
            </div>
            <p className="text-xs text-white/20 uppercase tracking-widest text-right max-w-50">
      ...
      <section className="py-24 px-6 md:px-16 border-t border-white/8">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-start">
          <div data-reveal>
            <p className="text-xs text-white/30 uppercase tracking-widest mb-6">À propos</p>
            <p className="text-3xl md:text-4xl font-serif leading-snug text-white/90">
      ...
      <section className="py-24 px-6 md:px-16 border-t border-white/8">
        <div className="max-w-7xl mx-auto">
          <p data-reveal className="text-xs text-white/30 uppercase tracking-widest mb-12">
            Parcours
          </p>
          <div>
            {experiences.map((exp, i) => (
              <div
                key={i}
                data-reveal
                className="group grid grid-cols-1 md:grid-cols-[180px_1fr_auto] gap-4 md:gap-8 py-8 border-t border-white/8 hover:bg-white/2 -mx-6 px-6 transition-colors"
              >
                <div>
                  <p className="text-xs text-white/30 uppercase tracking-widest mt-1">{exp.period}</p>
                </div>
                <div>
                  <div className="flex items-baseline gap-3 mb-2">
                    <h3 className="text-lg font-semibold text-white">{exp.role}</h3>
                    <span className="text-sm text-white/30">· {exp.company}</span>
                  </div>
                  <p className="text-white/40 leading-relaxed text-sm">{exp.description}</p>
                </div>
                <div>
                  <span className="text-xs px-2 py-1 rounded-full border border-white/10 text-white/30">{exp.tag}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row gap-12 items-start">
            <div data-reveal className="md:w-1/3">
              <p className="text-xs text-white/30 uppercase tracking-widest mb-4">En dehors du code</p>
            ...
            <div data-reveal className="flex-1 grid grid-cols-2 gap-3">
              {hobbies.map((h) => (
                <div
                  key={h.label}
                  className="flex items-center gap-4 rounded-xl border border-white/8 p-5 hover:border-white/20 transition-colors"
                >
                  <span className="text-white/50">{h.icon}</span>
          ...
          <div data-reveal className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
            {links.map((l) => (
              <a
                ...
                className="group flex flex-col justify-between rounded-2xl border border-white/10 p-6 hover:border-white/30 hover:bg-white/4 transition-all min-h-32"
              >
                <p className="text-xs text-white/30 uppercase tracking-widest">{l.label}</p>
                <div className="flex items-end justify-between mt-4">
                  <p className="font-medium text-white/80 group-hover:text-white transition-colors">{l.value}</p>
                  <span className="text-xl text-white/20 group-hover:text-white group-hover:translate-x-0.5 transition-all">
                    {l.arrow}
                  </span>
                </div>
              </a>
      ...
      <footer className={`border-t border-white/8 ${spaceGrotesk.className}`}>
        <div className="flex items-center justify-between px-6 md:px-16 py-16">
          <a
            href="/competences"
            className="text-2xl md:text-3xl font-semibold uppercase tracking-tight text-white/20 hover:text-white transition-colors"
          >
            &larr; Compétences
          </a>
          <a
        ...
        <div className="flex flex-col md:flex-row items-center justify-between px-6 md:px-16 py-6 border-t border-white/8 gap-3">
          <p className="text-xs text-white/20 uppercase tracking-widest">
            &copy; {new Date().getFullYear()} Rafael Teixeira — Tous droits réservés
          </p>
          <p className="text-xs text-white/20 uppercase tracking-widest">Développeur Full Stack &amp; UI/UX</p>
        </div>
      </footer>
    </main>
  );
